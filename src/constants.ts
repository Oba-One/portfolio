export const projects = {
  ...
  freeport: {
    ...
    architecture:
      'Given the projects aim on giving art/asset ownership the SEC and regulations played a huge role in how the app could function along with the partners needed to meet compliance. The frontend focus was on creating a personal and engaging experience for the user enabling them to view their assets and have DeFi utility to take out liquidity or view them in a fun 3D manner. On the backend was a Node.js api handling authentication of users with SIWE (Sign in with Ethereum) and connecting with external providers to place buy/sell orders on assets.',
    development:
  ...
  mira_flow: {
    ...
    architecture:
      "Given this was a new platform being added to the product it was important to key in on the best tools to use given the team's skillset and urgency to release an MVP. Through discussions React Native was settled upon as the best option enabling the web and unity teams to contribute, with unity being able to write native modules React Native could hook in to. At the business logic level we had to transfer our workflow/compliance logic into Typescript. Within this process some difference between how workflows should function on the headset vs mobile device emerged with tweaks being made to accommodate each platform. The final stack consisted of a Node.js API, React Native client with Swift modules for data queueing/uploading.",
    development:
  ...
  gentle_monster: {
    ...
    architecture:
      'Joining the project the stack had been set as python on the backend and javascript on the front, with GraphQL connecting the two. Being on the frontend the initial focus was determining how GraphQL would be integrated, settling with Apollo as the client. The UI given it emphasis on images was architected to lazyload images and videos to improve performance.',
    development:
      ' I joined the team about a week into development with the initial backend being the only work completed. The 1st task was integrating GraphQL with Apollo due to its comprehensive tooling. With the data layer completed I switched focus to the UI creating the home, product detail, and products pages. The home page was mainly a use of flexbox and carousels for product viewing with bold full screen heros throughout. The products page was a grid layout using flexbox (at the time the css-grid had partial support) and a trailing animation. The product detail page was the more complex with a two column layout with the left being scrollable and right fixed. Lastly was integrating the payment flow with the backend which due to graphQL was pretty seamless.',
    learnings:
